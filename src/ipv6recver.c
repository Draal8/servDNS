...
void send_back(struct sockaddr_in6 client, char *message);
...
int main() {
	...
	my_addr.sin6_port = htons(lport);
	CHECK(inet_pton(AF_INET6, "::1", &my_addr.sin6_addr.s6_addr));
...
void send_back(struct sockaddr_in6 client, char *message) {
	...
	printf("IP address is: %s\n", inet_ntop(AF_INET6, &client.sin6_addr, adr, STR_SIZE));
	printf("port is: %d\n", (int) ntohs(client.sin6_port));
...
void domtab_create() {
	tab_null();
	domaine_create("fr", "::1", 3501);
	domaine_create("com", "::1", 3502);
	domaine_create("eu", "::1", 3503);
	domaine_create("net", "::1", 3504);
	domaine_create("org", "::1", 3505);
}
