...
int sender(char *site, char *adrs_port) {
	...
	CHECK(setsockopt(sockfd, SOL_SOCKET, SO_RCVTIMEO, &time,
	...
	i = 0;
	do {
	CHECK(sendto(sockfd, message, strlen(message)+1, 0,
...
//fonction recursive qui gere tous els erveurs sauf ceux racines
int suite(int sockfd, char *old_buff, char *site, struct sockaddr_in6 *dest, struct sockaddr_in6 *retour) {
	...
	int ret = 0, i, a = 0;
	char copy[STR_SIZE], *tmp, *adrs, *port, *saveptr, *message, *buff,
	adrs_port[STR_SIZE];
	socklen_t addrlen;
	if ((buff = malloc(STR_SIZE)) == NULL) rerror("malloc");
	
	tourniquet_suite(old_buff, 0);	//tourniquet de la fonction suite
	while ((tmp = tourniquet_suite(NULL, 0)) != NULL && ret != 1) {
		...
		dest->sin6_port = htons(atoi(port));
		//dest->sin6_addr = adrs;
		//normallement on ferait la meme chose pour l'addresse mais
		//inutile car on reste en local
		addrlen = sizeof(struct sockaddr_in6);
		...
		do {
			...
			recvfrom(sockfd, buff, STR_SIZE, 0, (struct sockaddr *) retour,
			&addrlen);
			i++;
		} while ((errno == EAGAIN || errno == EWOULDBLOCK) && i < 3);
		//si ya timeout on reessaye 3 fois
		
		if (errno == EAGAIN || errno == EWOULDBLOCK) {
			errno = 0;
			printf("timeout\n");
			register_time(buff, adrs_port, 1);	//1 = timeout
			continue;
		}
		printf("reponse : %s\n", buff);
		...
		for (i = 0; i < (int)strlen(buff); i++) {
			if (buff[i] == '-' && i < (int)strlen(buff)-1 && buff[i+1] == '1') {
				register_time(buff, adrs_port, 2);	//2 = pas trouve
				a = 1;
			}
		}
		if (a == 1) continue;
		register_time(buff, adrs_port, 0);	//0 = normal
		free(tmp);
...
//verifie si le nom de domaine voulu est atteint ou non
...
//construit le message a envoyer
